...
export class LabAnalyticsService {
  ...
  async aggregateLab(labId: string): Promise<void> {
    const [instances, submissions, flags, labSkills] = await Promise.all([
      ...
      this.prisma.labSkill.findMany({
        where: { labId },
        select: { skill: { select: { domain: { select: { displayName: true } } } } },
      }),
    ]);

    const totalAttempts = instances.length;
    const completions = instances.filter((i) => (i.status as string) === 'COMPLETED').length;
    ...
    const failureRate = totalSubmissions > 0 ? ((totalSubmissions - correctSubmissions) / totalSubmissions) * 100 : 0;

    // Calculate average time from completed instances
    const completedInstances = instances.filter((i) => (i.status as string) === 'COMPLETED');
    const avgTimeMinutes = completedInstances.length > 0
      ? completedInstances.reduce((sum, i) => {
          const timeMs = new Date(i.expiresAt).getTime() - new Date(i.createdAt).getTime();
    ...
    const stepAnalytics = flags.map((flag) => {
      ...
      const flagFailureRate = flagSubs.length > 0 ? ((flagSubs.length - flagCorrect) / flagSubs.length) * 100 : 0;
    ...
    const recentInstances = instances.filter((i) => new Date(i.createdAt) >= eightWeeksAgo);
    const weeklyCompletions = Array.from({ length: 8 }, (_, weekIdx) => {
      const weekStart = new Date(Date.now() - (7 - weekIdx) * 7 * 24 * 60 * 60 * 1000);
      const weekEnd = new Date(Date.now() - (6 - weekIdx) * 7 * 24 * 60 * 60 * 1000);
      const count = recentInstances.filter(
        (i) => (i.status as string) === 'COMPLETED' && new Date(i.createdAt) >= weekStart && new Date(i.createdAt) < weekEnd,
      ).length;
    ...
    const lab = await this.prisma.lab.findUnique({ where: { id: labId }, select: { difficulty: true } });
  ...
  async getLabInsights(labId: string): Promise<Any> {
    const analytics = await this.prisma.labAnalytics.findUnique({
      where: { labId },
      include: { lab: { select: { title: true, description: true, difficulty: true } } },
    });
    ...
    if (analytics.tooEasy) insights.push('This lab has a very high completion rate (>85%). Consider increasing difficulty or adding advanced flags.');
    if (analytics.tooHard) insights.push('This lab has a very low completion rate (<15%). Consider adding hints or simplifying early steps.');
    if (analytics.failureRate > 70) insights.push('High failure rate across flags. Students may need better prerequisites or more scaffolding.');
    if (analytics.avgTimeMinutes > (analytics.lab as Any).difficulty * 0.05) insights.push('Average completion time is high relative to difficulty. Consider simplifying the scenario.');

    const stepData = analytics.stepAnalytics as Array<{ step: string; failureRate: number }> | null;
    if (stepData) {
      const hardestStep = stepData.reduce((max, s) => s.failureRate > max.failureRate ? s : max, stepData[0]);
      if (hardestStep && hardestStep.failureRate > 60) {
        insights.push(`Step "${hardestStep.step}" has ${hardestStep.failureRate}% failure rate — consider adding hints.`);
      }
    }
