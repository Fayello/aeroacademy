...
export class CrossDomainService {
  ...
  async joinMission(userId: string, missionId: string) {
    const mission = await this.prisma.crossDomainMission.findUnique({ where: { id: missionId } });
    if (!mission) throw new NotFoundException('Mission not found');

    const now = new Date();
    if (now > mission.expiresAt) throw new BadRequestException('Mission has expired');
    ...
    const requiredDomains = mission.requiredDomains as Array<{ domainId: string; minSkillXp: number }>;
  ...
  async updateDomainProgress(userId: string, missionId: string, domainId: string, xp: number) {
    ...
    const requiredDomains = mission.requiredDomains as Array<{ domainId: string; minSkillXp: number }>;
    const allMet = requiredDomains.every(rd => (progress[rd.domainId] ?? 0) >= rd.minSkillXp);

    return this.prisma.crossDomainProgress.update({
      where: { id: participant.id },
      data: {
        progress,
        ...(allMet && !participant.completed ? { completed: true, completedAt: new Date() } : {}),
      },
    });
  }
  ...
  async claimReward(userId: string, missionId: string) {
    ...
    if (!participant) throw new NotFoundException('Not participating');
    if (!participant.completed) throw new BadRequestException('Mission not yet completed');
    ...
    this.logger.log(`User ${userId} claimed cross-domain reward: ${mission.xpReward} XP`);
