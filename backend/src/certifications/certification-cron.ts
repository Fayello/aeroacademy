    ...
    try {
      ...
      for (const user of users) {
        try {
          ...
          if (codes.length > 0) {
            this.logger.log(
              `Auto-awarded ${codes.join(', ')} to ${user.email}`,
            );
      ...
      this.logger.log(
        `Daily certification check complete. ${awarded} certifications auto-awarded.`,
      );
    } catch (error) {
      this.logger.error('Failed to run daily certification evaluation', error);
    }
  }
}
