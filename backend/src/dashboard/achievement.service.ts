...
export const ACHIEVEMENT_DEFINITIONS = [
  ...
  { title: 'FLAG_HUNTER', description: 'Capture 10 flags', icon: 'Crosshair', category: 'MILESTONE', rarity: 'UNCOMMON', requirementType: 'FLAGS_CAPTURED', requirementTarget: 10, xpReward: 150, chainParentId: 'FIRST_BLOOD' },
  { title: 'FLAG_VETERAN', description: 'Capture 50 flags', icon: 'Crosshair', category: 'MILESTONE', rarity: 'RARE', requirementType: 'FLAGS_CAPTURED', requirementTarget: 50, xpReward: 500, chainParentId: 'FLAG_HUNTER' },
  { title: 'FLAG_LEGEND', description: 'Capture 100 flags', icon: 'Crosshair', category: 'MILESTONE', rarity: 'EPIC', requirementType: 'FLAGS_CAPTURED', requirementTarget: 100, xpReward: 1000, chainParentId: 'FLAG_VETERAN' },
  { title: 'FLAG_GOD', description: 'Capture 500 flags', icon: 'Crosshair', category: 'MILESTONE', rarity: 'LEGENDARY', requirementType: 'FLAGS_CAPTURED', requirementTarget: 500, xpReward: 5000, chainParentId: 'FLAG_LEGEND' },
  ...
  { title: 'PENTEST_APPRENTICE', description: 'Complete 5 labs', icon: 'Shield', category: 'MILESTONE', rarity: 'UNCOMMON', requirementType: 'LABS_COMPLETED', requirementTarget: 5, xpReward: 250, chainParentId: 'FIRST_STRIKE' },
  { title: 'PENTEST_OPERATIVE', description: 'Complete 15 labs', icon: 'Shield', category: 'MILESTONE', rarity: 'RARE', requirementType: 'LABS_COMPLETED', requirementTarget: 15, xpReward: 750, chainParentId: 'PENTEST_APPRENTICE' },
  { title: 'PENTEST_COMMANDER', description: 'Complete 30 labs', icon: 'Shield', category: 'MILESTONE', rarity: 'EPIC', requirementType: 'LABS_COMPLETED', requirementTarget: 30, xpReward: 2000, chainParentId: 'PENTEST_OPERATIVE' },
...
export class AchievementService {
  ...
  async seedAchievements() {
    for (let i = 0; i < ACHIEVEMENT_DEFINITIONS.length; i++) {
      ...
      if (def.chainParentId) {
        const parent = await this.prisma.achievement.findUnique({ where: { title: def.chainParentId } });
  ...
  async getAchievementProgress(userId: string) {
    ...
    return allAchievements.map((ach) => {
      ...
      const progress = userAch ? userAch.progress : this.calculateProgress(ach, stats);
  ...
  async checkAndUnlockAchievements(userId: string) {
    ...
    for (const ach of allAchievements) {
      ...
      if (progress >= ach.requirementTarget) {
        ...
        logger.log(`Unlocked "${ach.title}" for user ${userId} (${progress}/${ach.requirementTarget})`);
        ...
        if (progress > 0) {
          await this.prisma.userAchievement.upsert({
            ...
            update: { progress },
            create: { userId, achievementId: ach.id, progress, target: ach.requirementTarget },
          }).catch(() => {});
        }
      }
    }
  ...
  private async getUserStats(userId: string) {
    const [flagsCaptured, labsCompleted, lessonsCompleted, quizzesPassed, user, missionCount, weeklyCount, coursesEnrolled, coursesCompleted, referrals, teamJoined, discussionPosts, discussionComments] = await Promise.all([
      ...
      this.prisma.user.findUnique({ where: { id: userId }, select: { xp: true, currentStreak: true, longestStreak: true, teamId: true, referredBy: true } }),
      ...
      this.prisma.courseEnrollment.findMany({
        ...
        for (const { courseId } of enrollments) {
          ...
          const done = await this.prisma.progress.count({ where: { userId, completed: true, lesson: { section: { courseId } } } });
          if (total > 0 && done >= total) completed++;
        }
        return completed;
      }),
      this.prisma.user.count({ where: { referredBy: userId } }),
      this.prisma.user.findUnique({ where: { id: userId }, select: { teamId: true } }).then((u) => (u?.teamId ? 1 : 0)),
  ...
  private calculateProgress(ach: { requirementType: string; requirementTarget: number }, stats: Record<string, number>): number {
