...
export class DashboardService implements OnModuleInit {
  ...
  async getSystemIntelligence(userId?: string) {
    ...
    const [totalUsers, totalLabs, activeContainers] = await Promise.all([
      ...
      this.docker.listContainers({ all: true }).then(containers => 
        containers.filter(c => c.Names.some(n => n.includes('/lab-')) && c.State === 'running').length
      ).catch(() => 0),
    ]);
    ...
    try {
      ...
      const labContainers = containers.filter(c => c.Names.some(n => n.includes('lab-')));
      ...
      networkLoad = Math.min(Math.round((totalRxBytes / (1024 * 1024)) * 100), 100);
    } catch (err) {
      // Ignore
    }
    ...
    return {
      ...
      stats: {
        ...
        completedLessons: await this.prisma.progress.count({ where: { completed: true } }),
        maxCapacity,
      }
    };
  }
  ...
  async getLabTelemetry() {
    try {
      ...
      const telemetry = await Promise.all(labContainers.map(async (container) => {
        const stats = await this.docker.getContainer(container.Id).stats({ stream: false });
        ...
        const memPercent = (stats.memory_stats.usage / stats.memory_stats.limit) * 100.0;
        ...
        return {
          ...
          network: Math.round((stats.networks?.eth0?.rx_bytes || 0) / 1024) || 0, // KB
